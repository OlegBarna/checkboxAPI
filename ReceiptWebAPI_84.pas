...
type
  ...
  private
    ...
    function ExecuteCommand(const ACommand: string; const AParameters: array of string; out AOutput: string): Integer;
    function CopyFile(const ASource, ADestination: string): Boolean;
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    SetLength(Goods[I].Good.Tax, GoodTaxesArray.Count);
                    for J := 0 to GoodTaxesArray.Count - 1 do
                      Goods[I].Good.Tax[J] := GoodTaxesArray.Items[J].AsInteger;
                  end;
                end;
...
begin
  ...
  LSaveFile := IncludeTrailingPathDelimiter(FReceiptsDirectory) + 'receipt_' + AReceiptId + '_' + IntToStr(GetTickCount) + '.html';
  ...
  // Логуємо команду
  Log('⚡ [GetReceiptHTML] /receipts/' + AReceiptId + '/html');
  Log('Executing curl command: ' + LCommand);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        try
          ...
          if CopyFile(LOutputFile, LSaveFile) then
          begin
            Log('✅ HTML збережено: ' + ExtractFileName(LSaveFile));
          end;

          Log('✅ HTML успішно отримано: ' + LOutputFile);
        finally
          LFile.Free;
        end;
      ...
      begin
        FLastError := 'HTML файл не було створено';
        Log('✗ ' + FLastError);
      end;
    end
    else
    begin
      FLastError := 'Помилка виконання curl: код ' + IntToStr(LExitCode);
      Log('✗ Помилка отримання HTML: ' + FLastError);
    end;
  except
    on E: Exception do
    begin
...
begin
  ...
  // Логуємо команду
  Log('⚡ [GetReceiptPNG] /receipts/' + AReceiptId + '/png');
  Log('Executing curl command: ' + LCommand);
  ...
  try
    ...
    begin
      Result := True;
      Log('✅ PNG успішно отримано: ' + AFileName);
    ...
    begin
      FLastError := 'Помилка виконання curl: код ' + IntToStr(LExitCode);
      Log('✗ Помилка отримання PNG: ' + FLastError);
    end;
  except
    on E: Exception do
    begin
...
begin
  ...
  LSaveFile := IncludeTrailingPathDelimiter(FReceiptsDirectory) + 'receipt_' + AReceiptId + '_' + IntToStr(GetTickCount) + '.txt';
  ...
  // Логуємо команду
  Log('⚡ [GetReceiptText] /receipts/' + AReceiptId + '/text');
  Log('Executing curl command: ' + LCommand);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        try
          ...
          if CopyFile(LOutputFile, LSaveFile) then
          begin
            Log('✅ Текст збережено: ' + ExtractFileName(LSaveFile));
          end;

          Log('✅ Текст успішно отримано: ' + LOutputFile);
        finally
          LFile.Free;
        end;
        DeleteFile(LOutputFile); // Видаляємо тимчасовий файл
      ...
      begin
        FLastError := 'Текстовий файл не було створено';
        Log('✗ ' + FLastError);
      end;
    end
    else
    begin
      FLastError := 'Помилка виконання curl: код ' + IntToStr(LExitCode);
      Log('✗ Помилка отримання тексту: ' + FLastError);
    end;
  except
    on E: Exception do
    begin
...
begin
  ...
  // Логуємо команду
  Log('⚡ [GetReceiptQRCode] /receipts/' + AReceiptId + '/qrcode');
  Log('Executing curl command: ' + LCommand);
  ...
  try
    ...
    begin
      Result := True;
      Log('✅ QR-код успішно отримано: ' + AFileName);
    ...
    begin
      FLastError := 'Помилка виконання curl: код ' + IntToStr(LExitCode);
      Log('✗ Помилка отримання QR-коду: ' + FLastError);
      Result := False;
    end;
  except
    on E: Exception do
...
function TReceiptWebAPI.ExecuteCommand(const ACommand: string; const AParameters: array of string; out AOutput: string): Integer;
var
  LProcess: TProcess;
...
begin
  ...
  try
    LProcess.Executable := ACommand;
    LProcess.Parameters.AddStrings(AParameters);
    ...
    // Читаємо вивід процесу
    repeat
      BytesRead := LProcess.Output.Read(Buffer, SizeOf(Buffer));
      if BytesRead > 0 then
        LOutputStream.Write(Buffer, BytesRead);
    until BytesRead = 0;
...
function TReceiptWebAPI.CopyFile(const ASource, ADestination: string): Boolean;
var
...
begin
  ...
  try
    ...
    try
      LDestStream := TFileStream.Create(ADestination, fmCreate);
      try
        LDestStream.CopyFrom(LSourceStream, 0);
        Result := True;
      finally
        LDestStream.Free;
      end;
    ...
    on E: Exception do
    begin
      Log('✗ Помилка копіювання файлу: ' + E.Message);
      Result := False;
    end;
  end;
end;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              // ВИДАЛЕНО: спеціальна обробка помилок податкових груп
              // ЗАЛИШЕНО: логування інших помилок валідації
              Log('⚠️ ПОМИЛКА ВАЛІДАЦІЇ: ' + ErrorMsg);
              Log('📍 Поле: ' + FieldPath);

              // Для всіх помилок валідації не робимо повторних спроб
              ShouldRetry := False;
